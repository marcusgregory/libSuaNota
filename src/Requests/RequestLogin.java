...
//Método "request" recebe um objeto "Sistema" e faz o Login do usuário usando o CPF e retorna um objeto "Usuario"
    ...
    public static Usuario request(Sistema sistema) throws ErroLoginException, IOException {
        ...
        Connection.Response pagConsultaID = Jsoup.connect("https://www.sefaz.ce.gov.br/content/aplicacao/internet/suanota/consultas/consulta_ID.asp")
                ...
                .data("txtCpfCnpj", sistema.getCPF())//CPF/CNPJ do Usuário.
        ...
        //ou não existe cadastro no Sistema da Sefaz.
        if (pagConsultaID.parse().select("#textoContainer > form > p:nth-child(1) > span").isEmpty()) {
            ...
            Connection.Response pagLogin = Jsoup.connect("https://www.sefaz.ce.gov.br/content/aplicacao/internet/suanota/digitacao_online/validar_usuario.asp")
                    ...
                    .followRedirects(false)//permite que a página não seja redirecionada
                    .execute();
            ...
            if (pagLogin.statusCode() == 200) {
                ...
                Elements tabUsuario = pagLogin.parse().select("form>table td:eq(1)");//Tabela onde se encontra o nome,cpf e o identificador.
